...
#' @param mappedpt data frame received from function `map_methyl` containing relative genomic position (pos), pseudo time of the cells (pt) and methylation status (methyl) -1 or 1 (un-/methylated).
...
#' @export
#'
#' @examples \dontrun{smooth2d(mappedpt, 400, 0.8, 40, 0.08,  xrange = range(mappedpt$pos), trange=range(mappedpt$pt)}

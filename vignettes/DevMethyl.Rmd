...
```{r meta}
metapath <- file.path( system.file("extdata",package="DevMethyl", "cell-annotation-short.tsv" ) )
...
header <- readLines(system.file("extdata", "column_header.txt", package = "DevMethyl")) %>% 
  ...
  header <- header %>%
          dplyr::mutate(cell_id = sub("_(?!A).*", "",cell_id, perl = TRUE))
...
```{r gauss_kernel}
m <- DevMethyl::smooth(mappedpt, hx, ht, delx, delt,  xrange = range(mappedpt$pos), trange = range(mappedpt$pt))
```
...
```{r accessibility}
# use example numpy file of GpC methylation (chromatin accessibility)
accpath <- file.path( system.file("extdata",package="DevMethyl", "8_GpC_trunc.npz" ) )
...
mAcc <-  DevMethyl::smooth(mappedpt_acc, hx, ht, delx, delt)
```
...
```{r plot_CpGi, eval=FALSE}
plot_cpgislands(species, genome, chr, ROIstart, ROIend)
```
```{r plot_CpGi_hidden, echo=FALSE, fig.width=7}
tryCatch(
  {
    plot_cpgislands(
      species = species,
  ...
  error = function(e) {
    message(
      "The UCSC request failed; using the bundled example data."
    )

    readRDS(
      system.file(
        "extdata",
        "example_cpgislands_plot",
        package = "DevMethyl"
      )
    )
  }
...
```{r get_cpgs, eval=FALSE}
get_cpgs(species, genome, chr, ROIstart, ROIend) %>% head()
```
```{r get_cpgs_hidden, echo=FALSE}
tryCatch(
  {
    get_cpgs(
      species = species,
  ...
  error = function(e) {
    message(
      "The Ensembl request failed; using the bundled example data."
    )
...
```{r plot_CpGs, eval=FALSE}
plot_cpgs(species, genome, chr, ROIstart, ROIend, n_bins = 50)
```
```{r plot_CpGs_hidden, echo=FALSE, fig.width=7}
tryCatch(
  {
    plot_cpgs(
      species = species,
...
```{r genemodel, eval=FALSE}
genepath <- rtracklayer::readGFF("https://ftp.ensembl.org/pub/release-102/gtf/mus_musculus/Mus_musculus.GRCm38.102.gtf.gz")

get_genemodel(genepath, chr, ROIstart, ROIend) -> genemodel

  subset(genemodel,select = c(1:5,7,11))
```
```{r genemodel_hidden, echo=FALSE, fig.width=12}
genepath <- rtracklayer::readGFF("https://ftp.ensembl.org/pub/release-102/gtf/mus_musculus/Mus_musculus.GRCm38.102.gtf.gz")
...
```{r plot_genemodel, eval=FALSE}
plot_genemodel(genepath, chr, ROIstart, ROIend) 
```
```{r plot_genemodel_hidden, echo=FALSE, fig.width=7}
tryCatch(
  {
    plot_genemodel(
      genepath = genepath,
...
```{r reg_features, eval=FALSE}
regpath <- rtracklayer::readGFF("https://ftp.ensembl.org/pub/release-102/regulation/mus_musculus/mus_musculus.GRCm38.Regulatory_Build.regulatory_features.20180516.gff.gz")

get_regfeat(regpath, chr, ROIstart, ROIend) -> regfeat

  subset(regfeat, select = c(1,4,5,10,11,13))
```
```{r reg_features_hidden, echo=FALSE}
regpath <- rtracklayer::readGFF("https://ftp.ensembl.org/pub/release-102/regulation/mus_musculus/mus_musculus.GRCm38.Regulatory_Build.regulatory_features.20180516.gff.gz")
...
```
```{r plot_features_hidden, echo=FALSE, fig.width=7}
tryCatch(
  {
    plot_regfeat(
      regpath = regpath,
...
```
```{r plot_all_hidden, echo=FALSE, fig.height=8, fig.width=10}
tryCatch(
  {
    plot_all(species, genome, sp, spAcc, meta, header, header_acc, genepath, regpath, chr, ROIstart, ROIend, hx, ht, VMRstart, VMRend, n_bins = 40)
  ...
  error = function(e) {
    message(
      "The request has failed. Possibly caused by HTTP 500 error; using the bundled example data."
    )

    readRDS(
      system.file(
        "extdata",
        "example_summary_plot_small.rds",
        package = "DevMethyl"
      )
    )
  }
